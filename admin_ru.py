...
async def команда_инновационный_старт(чат_ид, пользователь):
    приветствие = f"""🌟 <b>ДОБРО ПОЖАЛОВАТЬ</b> 🌟
<i>🚀AI CHAT 2.0 🚀</i>

┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃  💫 <b>{пользователь}</b>, вас приветствует    ┃
┃      <b>БУДУЩЕЕ ИСКУССТВЕННОГО</b>     ┃
┃           <b>ИНТЕЛЛЕКТА!</b>           ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🧠 <b>НОВЫЕ ВОЗМОЖНОСТИ:</b>
┌────────────────────────────-
│ 🤖 GPT-4o — Мультимодельный AI  │
│ 🎨 DALL-E 3 — Генерация арта    │
│ 🔊 Whisper — Голосовой ввод      │
│ 📊 Аналитика — Продвинутая       │
│ 💎 Персонализация — Умная        │
└─────────────────────────────┘

⚡ <b>МГНОВЕННЫЙ СТАРТ:</b>
Просто напишите любой вопрос и
испытайте мощь современного AI!

🎯 <b>БЫСТРЫЕ КОМАНДЫ:</b>
• /help — Подробная помощь
• /stats — Ваша статистика

✨ <i>Готовы изменить мир с AI?</i>"""
    ...
    кнопки = [
        ...
        [{"text": "❓ Помощь", "callback_data": "help"}, 
         {"text": "💰 PLUS ", "callback_data": "upgrade_subscription"}]
    ]
...
async def отправить_с_кнопками(чат_ид, текст, кнопки):
    """Отправить сообщение с инлайн кнопками с улучшенной обработкой ошибок"""
    try:
        import httpx
        данные = {
            "chat_id": чат_ид,
        ...
        лог.info(f"📤 Отправка сообщения с {len(кнопки)} рядами кнопок в чат {чат_ид}")
        
        async with httpx.AsyncClient(timeout=15.0) as клиент:
            ответ = await клиент.post(url, json=данные)
            
            if ответ.status_code == 200:
                лог.info(f"✅ Сообщение с кнопками успешно отправлено в чат {чат_ид}")
                return True
            else:
                лог.error(f"❌ Ошибка отправки сообщения: {ответ.status_code} - {ответ.text}")
                # Попробуем отправить без кнопок как fallback
                await отправить_сообщение(чат_ид, текст)
                return False
                
    except Exception as е:
        лог.error(f"💥 Критическая ошибка отправки с кнопками: {е}")
        try:
            # Fallback - отправляем без кнопок
            await отправить_сообщение(чат_ид, текст)
        except:
            лог.error("💥 Fallback отправка также не удалась")
        return False
...
async def обработать_подписку(чат_ид, тип_подписки):
    """Обработать запрос на подписку"""
    цены = {
        "PRO": {"цена": 299, "валюта": "RUB", "описание": "⭐ PRO подписка"},
        "TEAM": {"цена": 999, "валюта": "RUB", "описание": "🏆 TEAM подписка"},
        "ENTERPRISE": {"цена": 2999, "валюта": "RUB", "описание": "💼 ENTERPRISE подписка"}
    }
    
    if тип_подписки in цены:
        инфо = цены[тип_подписки]
        сообщение = f"""💳 <b>{инфо['описание']}</b>

💰 <b>Стоимость:</b> {инфо['цена']} {инфо['валюта']}/месяц

🔮 <b>Система платежей Telegram будет доступна в следующем обновлении!</b>

📧 <b>Сейчас вы можете:</b>
• Обратиться к администратору @admin
• Написать на email: support@bot.ru
• Или дождаться автоматизации платежей

✨ <i>Мы работаем над интеграцией Telegram Payments для вашего удобства!</i>"""
        
        кнопки = [
            [{"text": "📧 Связаться с админом", "url": "https://t.me/admin"}],
            [{"text": "🔄 Выбрать другой тариф", "callback_data": "upgrade_subscription"}],
            [{"text": "◀️ Назад", "callback_data": "back_to_main"}]
        ]
        
        await отправить_с_кнопками(чат_ид, сообщение, кнопки)
    else:
        await отправить_сообщение(чат_ид, 
            f"❌ Неизвестный тип подписки: {тип_подписки}")

async def ответить_на_callback(callback_id, текст="", показать_алерт=False):
    """Ответить на callback query с улучшенной обработкой ошибок"""
    try:
        import httpx
        данные = {
            "callback_query_id": callback_id, 
            "text": текст, 
            "show_alert": показать_алерт or bool(текст)
        }
        url = f"https://api.telegram.org/bot{ТЕЛЕГРАМ_БОТ_ТОКЕН}/answerCallbackQuery"
        
        async with httpx.AsyncClient(timeout=10.0) as клиент:
            ответ = await клиент.post(url, json=данные)
            
            if ответ.status_code == 200:
                лог.info(f"✅ Callback {callback_id} успешно подтвержден")
                return True
            else:
                лог.error(f"❌ Ошибка подтверждения callback: {ответ.status_code} - {ответ.text}")
                return False
                
    except Exception as е:
        лог.error(f"💥 Критическая ошибка ответа на callback {callback_id}: {е}")
        return False
