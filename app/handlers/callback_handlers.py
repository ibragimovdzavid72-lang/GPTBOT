...
async def handle_set_language(callback_query: types.CallbackQuery, lang_code: str) -> None:
    ...
    try:
        ...
        if success:
            ...
            from ..main import get_text  # ленивый импорт, чтобы избежать цикла
...
async def handle_web_search_menu(callback_query: types.CallbackQuery) -> None:
    ...
    try:
        ...
        search_text = (
            f"🔍 <b>{(__import__('app.main', fromlist=['get_text']).get_text)('web_search', user_lang)}</b>\n\n"
            f"{(__import__('app.main', fromlist=['get_text']).get_text)('search_help', user_lang)}\n\n"
            f"📝 <b>{(__import__('app.main', fromlist=['get_text']).get_text)('search_placeholder', user_lang)}</b>\n"
        ...
        back_button = [[InlineKeyboardButton(
            text=(__import__('app.main', fromlist=['get_text']).get_text)("back", user_lang), 
...
async def handle_news_search_menu(callback_query: types.CallbackQuery) -> None:
    ...
    try:
        ...
        get_text = __import__('app.main', fromlist=['get_text']).get_text
        news_text = (
...
async def handle_toggle_versions_lang(callback_query: types.CallbackQuery) -> None:
    """Переключение языка на странице версий и сохранение выбора пользователя."""
    try:
        current_lang = await user_service.get_user_language(callback_query.from_user.id)
        new_lang = "en" if current_lang == "ru" else "ru"
        await user_service.set_user_language(callback_query.from_user.id, new_lang)
        await message_flow.show_pro_versions(callback_query, new_lang)
        await callback_query.answer()
    except Exception as e:
        logger.error(f"❌ Error in handle_toggle_versions_lang: {e}")
        await callback_query.answer("❌ Произошла ошибка", show_alert=True)


async def handle_show_welcome(callback_query: types.CallbackQuery) -> None:
    """Показать экран приветствия с учётом языка пользователя."""
    try:
        user_lang = await user_service.get_user_language(callback_query.from_user.id)
        await message_flow.show_welcome_screen(callback_query, user_lang)
        await callback_query.answer()
    except Exception as e:
        logger.error(f"❌ Error in handle_show_welcome: {e}")
        await callback_query.answer("❌ Произошла ошибка", show_alert=True)
...
CALLBACK_HANDLERS = {
    ...
    "toggle_versions_lang": handle_toggle_versions_lang,
    "show_welcome": handle_show_welcome,
