...
async def process_callback(callback_query: types.CallbackQuery) -> None:
    ...
    if callback_query.data in ["ai_agent_pro", "back_to_main", "change_language", "set_lang_ru", "set_lang_en"]:
...
async def process_voice_text_message(callback_query: types.CallbackQuery, text: str, voice_response: bool = False) -> None:
    ...
    if search_service.detect_search_intent(message.text, SEARCH_KEYWORDS):
        try:
            ...
            # Выполняем поиск
            search_results = await search_service.search_web(message.text, max_results=3)
