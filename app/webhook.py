            ...
            try:
                ...
                if expected_secret:
                    ...
                    if got_secret != expected_secret:
                        logger.warning("❌ Неверный WEBHOOK_SECRET в запросе")
                ...
                try:
                    ...
                    # Логируем, но всё равно возвращаем 200, чтобы Telegram не ретраил
                    logger.error(f"❌ Ошибка в обработчиках: {handler_error}")
                return web.Response(status=200)
                
            except Exception as e:
                logger.error(f"❌ Ошибка webhook: {e}")
                # Возвращаем 200 даже при общей ошибке, чтобы избежать 502/ретраев у Telegram
                return web.Response(status=200)
