    ...
    if "message" in обновление:
        ...
        if "text" in сообщение:
            ...
            if текст.startswith('/start'):
                ответ = f"🎉 Привет, {пользователь}!\n\n"\
                        f"🤖 Я ваш русский AI-ассистент на основе GPT-4o.\n\n"\
                        f"💬 Отправьте мне любое сообщение, и я отвечу!\n\n"\
                        f"✨ Возможности:\n"\
                        f"• Ответы на вопросы\n"\
                        f"• Помощь с задачами\n"\
                        f"• Общение на русском языке"
                await отправить_ответ_телеграм(чат_ид, ответ)
            elif текст.startswith('/help'):
                ответ = f"🎆 Помощь по использованию бота\n\n"\
                        f"🕹️ Команды:\n"\
                        f"/start - Начать работу\n"\
                        f"/help - Показать эту справку\n\n"\
                        f"💬 Просто напишите мне любое сообщение, и я отвечу!"
                await отправить_ответ_телеграм(чат_ид, ответ)
            else:
                # Обычное сообщение - отправляем на обработку в OpenAI
                ответ = await обработать_с_openai(текст, пользователь)
                await отправить_ответ_телеграм(чат_ид, ответ)
    
    лог.info(f"✅ Обновление {ид_обновления} обработано")

async def обработать_с_openai(текст, пользователь):
    """Обработка сообщения через OpenAI."""
    if not КЛЮЧ_OPENAI:
        return f"⚠️ Извините, OpenAI API не настроен. Пока я могу только отвечать на команды /start и /help."
    
    try:
        лог.info(f"🤖 Отправка запроса в OpenAI: {текст[:50]}...")
        
        # Простая заглушка для OpenAI запроса
        import httpx
        ...
        данные = {
            ...
            "messages": [
                {
                    "role": "system", 
                    "content": f"Ты - дружелюбный русский AI-ассистент. Отвечай на русском языке дружелюбно и полезно."
                },
                {
                    "role": "user", 
                    "content": текст
        ...
        async with httpx.AsyncClient(timeout=30.0) as клиент:
            ...
            if ответ.status_code == 200:
                ...
                ai_ответ = результат["choices"][0]["message"]["content"]
                лог.info(f"✅ OpenAI ответ получен: {ai_ответ[:50]}...")
                return ai_ответ
            else:
                лог.error(f"❌ Ошибка OpenAI API: {ответ.status_code} - {ответ.text}")
                return f"⚠️ Извините, произошла ошибка при обращении к OpenAI. Попробуйте позже."
                
    except Exception as е:
        лог.error(f"💥 Ошибка при обращении к OpenAI: {е}")
        return f"⚠️ Извините, произошла ошибка. Попробуйте позже."
...
@приложение.get("/test")
async def тест_маршрута():
    """Тестовый маршрут для проверки работы FastAPI."""
    return JSONResponse({
        "статус": "работает",
        "время": datetime.now().isoformat(),
        "сообщение": "Тестовый маршрут работает корректно"
    })

@приложение.post("/webhook/{secret_path}")
async def вебхук_с_секретом(запрос: Request, secret_path: str):
    """Webhook с секретом в пути."""
    лог.info(f"📨 Получен webhook с секретным путем: /{secret_path}")
    ...
    try:
        ...
        лог.info(f"🔍 Проверяем секретный путь '{secret_path}' против списка: {допустимые_секреты}")
        
        if secret_path not in допустимые_секреты:
            лог.warning(f"🚫 Неверный секретный путь: {secret_path}")
            лог.warning(f"🚫 Ожидался один из: {допустимые_секреты}")
            raise HTTPException(status_code=404, detail="Не найдено")
        
        лог.info(f"✅ Секретный путь принят: {secret_path}")
        ...
        данные = json.loads(тело.decode("utf-8"))
        ид_обновления = данные.get("update_id", "неизвестно")
        лог.info(f"✅ Обновление #{ид_обновления} через секретный путь {secret_path}")
