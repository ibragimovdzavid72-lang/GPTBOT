...
async def инициализировать_сервисы():
    ...
    try:
        ...
        if not настройки.телеграм.токен_бота or настройки.телеграм.токен_бота.startswith("ваш"):
            raise Exception("❌ TELEGRAM_BOT_TOKEN не установлен! Получите токен у @BotFather")
        
        if not настройки.опенаи.апи_ключ or настройки.опенаи.апи_ключ.startswith("ваш"):
            raise Exception("❌ OPENAI_API_KEY не установлен! Зарегистрируйтесь на platform.openai.com")
        
        if not настройки.база_данных.ссылка:
            raise Exception("❌ DATABASE_URL не установлен! Настройте подключение к PostgreSQL")
        ...
        if not await менеджер_бд.проверка_здоровья():
            raise Exception("❌ Ошибка проверки базы данных! Таблицы не созданы.")
